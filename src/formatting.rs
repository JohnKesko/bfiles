...
pub fn summarize_tree(tree: &DirTree, top_n: usize, child_limit: usize) -> TreeSummary {
        ...
        let mut groups: Vec<DirectoryGroup> = grouped
                ...
                .map(|(anchor, mut rows)| {
                        ...
                        let mut children: Vec<DirectoryRow> =
                                rows.into_iter().map(|(size, id)| DirectoryRow { relative_path: tree.relative_path_of(id), size }).collect();
                        children.sort_by(|a, b| b.size.cmp(&a.size).then_with(|| a.relative_path.cmp(&b.relative_path)));

                        DirectoryGroup { root: PathBuf::from(tree.name(anchor)), total_size: tree.size(anchor), children }
                })
                .collect();
        ...
        if root_files > 0 {
                groups.push(DirectoryGroup { root: PathBuf::from(ROOT_FILES_LABEL), total_size: root_files, children: Vec::new() });
        }
...
fn format_row(label: &str, size: u64, path_width: usize, size_width: usize) -> String {
        ...
        format!("{:<path_width$} {:>size_width$}", label, size_str, path_width = path_width, size_width = size_width)
}
...
mod tests {
        ...
        fn groups_descendants_under_their_top_level_directory() {
                ...
                assert_eq!(
                        ...
                        vec![PathBuf::from("andreasohlstrom.se").join("trends"), PathBuf::from("andreasohlstrom.se").join("trends").join(".next"),]
                );
                ...
                assert_eq!(
                        ...
                        vec![PathBuf::from("certbuddy").join(".next"), PathBuf::from("certbuddy").join(".next").join("dev")]
                );

                assert_eq!(groups[2].root, PathBuf::from("my plugin"));
                assert_eq!(groups[2].total_size, 700);
                assert_eq!(groups[2].children[0].relative_path, PathBuf::from("my plugin").join(".next"));
        }
