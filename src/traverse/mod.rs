...
pub(crate) fn is_excluded(excludes: &[PathBuf], parent: &Path, name: &OsStr) -> bool {
        excludes.iter().any(|excluded| excluded.file_name() == Some(name) && excluded.parent() == Some(parent))
}
...
mod tests {
        ...
        fn run_engines(base: &Path, excludes: &[PathBuf], check: impl Fn(&DirTree, u64)) {
                let engines: [&dyn TraversalEngine; 2] = [&crossbeam::CrossbeamTraversal, &rayon::RayonTraversal];
