...
mod portable {
        ...
        pub fn read_dir_entries(path: &Path) -> io::Result<Vec<Entry>> {
                ...
                for entry in rd.filter_map(Result::ok) {
                        ...
                        out.push(Entry { name: entry.file_name(), is_dir: file_type.is_dir(), is_symlink: file_type.is_symlink(), size });
                }
...
mod macos {
        ...
        unsafe extern "C" {
                fn getattrlistbulk(dirfd: c_int, attr_list: *mut Attrlist, attr_buf: *mut c_void, attr_buf_size: size_t, options: u64) -> c_int;
        }
        ...
        pub fn read_dir_bulk(path: &Path) -> io::Result<Vec<Entry>> {
                let cpath = CString::new(path.as_os_str().as_bytes()).map_err(|_| io::Error::from(io::ErrorKind::InvalidInput))?;
                ...
                BUF.with(|buf| {
                        ...
                        loop {
                                ...
                                let retcount = unsafe {
                                        getattrlistbulk(fd, &mut attrlist, buf.as_mut_ptr() as *mut c_void, buf.len() as size_t, FSOPT_PACK_INVAL_ATTRS)
                                };
        ...
        fn parse_entry(buf: &[u8], start: usize, out: &mut Vec<Entry>) -> io::Result<usize> {
                ...
                let name_start = (name_field as isize).checked_add(name_dataoffset).and_then(|v| usize::try_from(v).ok()).ok_or(io::ErrorKind::InvalidData)?;
                let raw = buf.get(name_start..name_start.checked_add(name_len).ok_or(io::ErrorKind::InvalidData)?).ok_or(io::ErrorKind::InvalidData)?;
...
mod tests {
        ...
        fn index(entries: Vec<super::Entry>) -> BTreeMap<String, (bool, bool, u64)> {
                entries.into_iter().map(|e| (e.name.to_string_lossy().into_owned(), (e.is_dir, e.is_symlink, e.size))).collect()
        }
